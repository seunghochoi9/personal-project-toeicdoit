...
from crime_util import Editor, Reader
...
class Crime_Service:
    ...
    def __init__(self):
        ...
        self.crime_all_columns = self.crime_columns + self.arrest_columns
    ...
    def save_police_position(self) -> pd.DataFrame:
        ...
        police_position = pd.merge(crime, stations, on='구별')
        police_position.to_csv(f'{self.data.sname}police_position.csv')



    def save_cctv_population(self) -> None:
        population = Reader().excel(f'{self.data.dname}pop_in_seoul.xls', 2, 'B, D, G, J, N')
        ...
        ic(population)
        ic(cctv.head(2))
        # population(how='all', inplace=True) # NaN 값이 모두 있는 행을 제거
        population.dropna(how='all', inplace=True)
        # 26번째 행을 삭제, axis=o 행을 삭제, axis=1 열을 삭제
        # population.drop([26], axis=0, inplace=True)
    ...
    def save_crime_arr_normalization(self) -> None:
        station_names = []
        crime = self.crime_dataframe()
        cctv = self.cctv_dataframe()
        ...
        crime['발생 총 건수'] = crime['강간 발생'] + crime['강도 발생'] + crime['살인 발생'] + crime['절도 발생'] + crime['폭력 발생']
        crime['검거 총 건수'] = crime['강간 검거'] + crime['강도 검거'] + crime['살인 검거'] + crime['절도 검거'] + crime['폭력 검거']
        crime['검거율'] = crime['검거 총 건수'] / crime['발생 총 건수'] * 100
        crime.drop(self.crime_all_columns, axis=1, inplace=True)
        crime.rename(columns={cctv.columns[0]: '구별'}, inplace=True)
        cctv.rename(columns={cctv.columns[0]: '구별'}, inplace=True)

        cctv_per_crime = pd.merge(cctv, crime, on='구별')
        cor1 = np.corrcoef(cctv_per_crime['검거율'], cctv_per_crime['소계'])

        print(f'검거율과 CCTV의 상관계수 {str(cor1)}')

    def save_crime_arrest_normalization(self) -> None:
        crime = self.crime_dataframe()
        reader = Reader()
        police_pos = reader.csv(f'{self.data.sname}police_position')    
        ...
        police['살인검거율'] = police['살인 검거'] / police['살인'] * 100
        police['강도검거율'] = police['강도 검거'] / police['강도'] * 100
        police['강간검거율'] = police['강간 검거'] / police['강간'] * 100
        police['절도검거율'] = police['절도 검거'] / police['절도'] * 100
        police['폭력검거율'] = police['폭력 검거'] / police['폭력'] * 100
        ...
        police_norm.to_csv(f'{self.data.sname}police_norm', sep=',', encoding='UTF-8')
...
if __name__ == "__main__":
    ...
    # ic(crime.save_cctv_population())
    # ic(crime.save_crime_arrest_normalization())
    # ic(crime.folium_test())
    ic(crime.draw_crime_map())
