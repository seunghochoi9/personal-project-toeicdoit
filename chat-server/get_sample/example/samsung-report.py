...
class SamsunReport:
    ...
    def __init__(self) -> None:
        ...
        self.data.dname = 'C:\\Users\\bitcamp\\kubernetes\\chat-server\\get_sample\\data\\'
    ...
    def preprocessing(self):
        ...
        with open(f'{self.data.dname}{self.data.fname}', 'r', encoding='utf-8') as f:
            texts = f.read()
        texts = texts.replace('\n', ' ') # 줄바꿈 제거
        tokenizer = re.compile(r'[^ㄱ-힣]+') # 한글만 추출
    ...
    def draw_wordcloud(self):
        ...
        ic(freqtext[:10])
        wcloud = WordCloud(font_path='./data/D2Coding.ttf', relative_scaling=0.2, background_color='white').generate_from_frequencies(freqtext)
...
jdk-19 환경에서 JPype, KoNLPy, WordCloud, Tweepy 설치
conda install -c conda-forge jpype1
pip install konlpy
conda install -c conda-forge wordcloud
conda install conda-forge::tweepy
__main__에서  nltk.download('punkt') 실행
...
어떤 것이 약어에 쓰이는 "."이고(Ex : Ph.D.), 어떤 것이 마침표인지 학습이 되어있다.
문장을 기본적으로 마침표를 기준으로 나누되, Ph.D., Saint., Professor., 와 같은 약어(Abbreviation)는
Known abbreviation으로 학습하여 한 단어로 취급하는 방식이다.
